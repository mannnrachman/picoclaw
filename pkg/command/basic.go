...
func (c *ShowCommand) Execute(ctx context.Context, agent AgentState, args []string, msg bus.InboundMessage) (string, error) {
	if len(args) < 1 {
		return "Usage: /show [model|channel|agents]", nil
	}
	switch args[0] {
	case "model":
	...
	case "agents":
		...
		if lister, ok := agent.(AgentLister); ok {
			agentIDs := lister.GetAgentIDs()
			return fmt.Sprintf("Registered agents: %s", strings.Join(agentIDs, ", ")), nil
		}
		return "Agent listing not supported", nil
	default:
...
func (c *ListCommand) Execute(ctx context.Context, agent AgentState, args []string, msg bus.InboundMessage) (string, error) {
	if len(args) < 1 {
		return "Usage: /list [models|channels|agents]", nil
	}
	switch args[0] {
	case "models":
